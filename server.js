...
const MAX_AVATAR_INVENTORY = 100;
const MAX_GAME_INVENTORY = 100;
...
function getPersistentSessionSecret() {
  ...
  try {
    ...
    // Si /var/data todavía no tiene el secreto (por ejemplo, al crear un
    // Persistent Disk nuevo), reutilizamos el secreto que viaja con el código.
    // Así un redeploy no invalida las sesiones existentes.
    const repoSecretPath = path.join(__dirname, "data", "session-secret.txt");
    if (fs.existsSync(repoSecretPath)) {
...
function saveUsersDisk(users) {
  ...
  try {
    ...
    if (fs.existsSync(usersPath)) {
      // No reemplazar un backup bueno por un users.json vacío/corrupto.
      try {
        const previous = readJsonObject(usersPath);
        if (Object.keys(previous).length > 0) fs.copyFileSync(usersPath, backupPath);
      } catch {}
    }
...
function ensureUserIds(users) {
  ...
  for (const [key, user] of Object.entries(users || {})) {
    ...
    if (Array.isArray(user.avatarInventory) && user.avatarInventory.length > MAX_AVATAR_INVENTORY) { user.avatarInventory = user.avatarInventory.slice(0, MAX_AVATAR_INVENTORY); changed = true; }
    if (!Array.isArray(user.gameInventory)) { user.gameInventory = []; changed = true; }
    if (Array.isArray(user.gameInventory) && user.gameInventory.length > MAX_GAME_INVENTORY) { user.gameInventory = user.gameInventory.slice(0, MAX_GAME_INVENTORY); changed = true; }
...
function publicUser(user, key) {
  ...
  return {
    ...
    avatar: user.avatar || {},
    avatarInventory: (user.avatarInventory || user.inventory || []).slice(0, MAX_AVATAR_INVENTORY),
    gameInventory: (user.gameInventory || []).slice(0, MAX_GAME_INVENTORY),
    inventory: [],
...
const server = http.createServer(async (req, res) => {
  ...
  if (urlPath === "/health") {
    ...
    return json(res, 200, {
      ...
      sessionSecretFile: fs.existsSync(sessionSecretPath),
      sessionsFile: fs.existsSync(sessionsPath),
  ...
  if (urlPath === "/api/me" && req.method === "POST") {
    ...
    if (Array.isArray(body.avatarInventory)) user.avatarInventory = body.avatarInventory.slice(0, MAX_AVATAR_INVENTORY);
    ...
    const ownedAvatarItems = new Set(
      (Array.isArray(user.avatarInventory) ? user.avatarInventory : []).map(id => String(id))
    );
    user.avatar.accessories = user.avatar.accessories
      .map(id => String(id))
      .filter((id, i, arr) => arr.indexOf(id) === i && ownedAvatarItems.has(id))
      .slice(0, 20);
    if (Array.isArray(body.gameInventory)) user.gameInventory = body.gameInventory.slice(0, MAX_GAME_INVENTORY);
