...
function ensureUserIds(users) {
  ...
  for (const [key, user] of Object.entries(users || {})) {
    ...
    if (!Array.isArray(user.avatarInventory)) {
      user.avatarInventory = Array.isArray(user.inventory) ? user.inventory.slice(0, 100) : [];
      changed = true;
    }
...
const server = http.createServer(async (req, res) => {
  ...
  if (urlPath === "/api/catalog/buy" && req.method === "POST") {
    ...
    const users = sess.users;
    const user = users[sess.key];
    user.avatarInventory = Array.isArray(user.avatarInventory) ? user.avatarInventory.map(String) : [];
    if (user.avatarInventory.includes(item.id)) {
      return json(res, 409, { error: "Ya tienes este articulo.", user: publicUser(user, sess.key) });
    }

    const price = Math.max(0, Math.floor(Number(item.price) || 0));
    const sunnys = Number(user.sunnys) || 0;
    if (sunnys < price) return json(res, 400, { error: "No tienes suficientes Sunnys." });
    if (user.avatarInventory.length >= 100) return json(res, 400, { error: "Tu inventario de avatar esta lleno." });

    // Compra atomica en servidor: primero se valida, luego se descuenta y se guarda.
    user.sunnys = sunnys - price;
    user.avatarInventory.push(item.id);
    user.inventory = [];
    users[sess.key] = user;
    saveUsersDisk(users);
  ...
  if (urlPath.startsWith('/assets/') || urlPath.startsWith('/templates/') || urlPath === '/favicon.png') {
