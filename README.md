...
## Sesiones y persistencia en Render

Para que las cuentas sobrevivan a cada redeploy, el servicio debe tener el Persistent Disk montado en `/var/data`. El código usa `EPICBLOXS_DATA_DIR=/var/data` para `users.json`, `sessions.json`, `catalog.json` y los demás datos.

Además, configura una variable de entorno privada llamada `SESSION_SECRET` en Render. Debe ser una cadena larga y aleatoria y **no debe cambiarse** después de crear las cuentas. El `render.yaml` la declara con `sync: false` para que Render la pida sin guardarla en GitHub.

Después de desplegar, abre `/health`. La respuesta debe mostrar `dataDir` como `/var/data`, `sessionSecretFile: true` o `hasSessionSecretEnv: true`, y un número de `accounts` mayor que 0.
